...
{
    public class InternalStateDataStore : StateDataStoreBase
    {
        ...
        /// </summary>
        public override event EventHandler<StateDataStoreUpdateEventArgs> StateUpdateNotify = delegate { };
        ...
        public override bool UpdateState(string key, object value)
        {
        ...
        /// </param>
        /// <returns></returns>
        public override string GetStates(string filter)
        {
            ...
            {
                var state = key + " :: " + _states[key];
                if (String.IsNullOrEmpty(filter) || state.ToLower().Contains(filter.ToLower())) // ToLower makes the comparison case insensitive
                {
                    states.Append(state + "\n");
                }
            }
        ...
        public override string GetState(List<string> keys)
        {
